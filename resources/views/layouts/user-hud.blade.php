...
<head>
    <meta charset="UTF-8">
    <title>@yield('title', 'MyTorrents')</title>
    ...
    <script>
        window.AUTH = {
            ...
            premium: @json(auth()->check() && (auth()->user()?->is_premium_active || auth()->user()?->is_trial)),
        };
    </script>
...
<footer class="text-center text-xs text-slate-500 py-10">
    © {{ date('Y') }} MyTorrents · Онлайн-кинотеатр · 18+
</footer>

{{-- ================= HUD MODALS ================= --}}
@includeIf('partials.hud-login')
@includeIf('partials.hud-premium')
